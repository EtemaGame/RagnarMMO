...
public class BlacksmithSkillEvents {
    ...
    public static void onBlacksmithAttack(LivingHurtEvent event) {
        if (event.getSource().getEntity() instanceof ServerPlayer player) {
            ...
            int researchLv = com.etema.ragnarmmo.skill.data.progression.SkillProgressManager.getProgress(player, new ResourceLocation("ragnarmmo", "research_weaponry")).getLevel();
            ...
            if (held.getItem() instanceof TieredItem tiered) {
                if (tiered.getTier() == Tiers.IRON || tiered.getTier() == Tiers.STONE) {
                    int ironLv = com.etema.ragnarmmo.skill.data.progression.SkillProgressManager.getProgress(player, new ResourceLocation("ragnarmmo", "iron_tempering")).getLevel();
                    multiplier += (ironLv * 0.01f);
                } else if (tiered.getTier() == Tiers.DIAMOND || tiered.getTier() == Tiers.NETHERITE) {
                    int steelLv = com.etema.ragnarmmo.skill.data.progression.SkillProgressManager.getProgress(player, new ResourceLocation("ragnarmmo", "steel_tempering")).getLevel();
                    multiplier += (steelLv * 0.015f);
                }
            }

            // 3. Unfair Trick (+level * 3% if target < 50% HP)
            int unfairLv = com.etema.ragnarmmo.skill.data.progression.SkillProgressManager.getProgress(player, new ResourceLocation("ragnarmmo", "unfair_trick")).getLevel();
    ...
    public static void onBlacksmithDefend(LivingDamageEvent event) {
        if (event.getEntity() instanceof ServerPlayer player) {
            int skinLv = com.etema.ragnarmmo.skill.data.progression.SkillProgressManager.getProgress(player, new ResourceLocation("ragnarmmo", "skin_tempering")).getLevel();
            if (skinLv > 0) {
