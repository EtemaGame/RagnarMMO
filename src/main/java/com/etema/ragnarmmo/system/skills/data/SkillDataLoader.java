...
public class SkillDataLoader extends SimpleJsonResourceReloadListener {
    ...
    protected void apply(Map<ResourceLocation, JsonElement> resources, ResourceManager resourceManager,
        ...
        int effectErrors = 0;
        for (SkillDefinition def : loadedFromJson.values()) {
            String effectClassName = def.getEffectClass();
            if (effectClassName == null || effectClassName.isEmpty()) {
                continue;
            }

            try {
                Class<?> clazz = Class.forName(effectClassName);
                if (com.etema.ragnarmmo.system.skills.ISkillEffect.class.isAssignableFrom(clazz)) {
                    com.etema.ragnarmmo.system.skills.ISkillEffect effect = (com.etema.ragnarmmo.system.skills.ISkillEffect) clazz
                            .getDeclaredConstructor().newInstance();
                    SkillRegistry.registerEffect(def.getId(), effect);
                    effectCount++;
                } else {
                    LOGGER.warn("Effect class {} does not implement ISkillEffect for skill {}",
                            effectClassName, def.getId());
                    effectErrors++;
                }
            } catch (ClassNotFoundException e) {
                LOGGER.warn("Effect class not found for skill {}: {}", def.getId(), effectClassName);
                effectErrors++;
            } catch (Exception e) {
                LOGGER.error("Failed to instantiate effect for skill {}: {}",
    ...
    private JsonObject applyFamilyDefaults(JsonObject skillJson) {
        ...
        var familyOpt = com.etema.ragnarmmo.system.skills.data.family.SkillFamilyRegistry.get(familyLoc);
    ...
    @SuppressWarnings("deprecation")
    private int getMaxLevelForSkill(SkillType type) {
        try {
            return com.etema.ragnarmmo.system.skills.SkillConfigManager.getMaxLevel(type.toResourceLocation());
        } catch (Exception e) {
            return isLifeSkillName(type.name()) ? 50 : 10;
        }
    }

    @SuppressWarnings("deprecation")
    private String getEffectClass(SkillType type) {
        String basePkg = "com.etema.ragnarmmo.system.skills.";
