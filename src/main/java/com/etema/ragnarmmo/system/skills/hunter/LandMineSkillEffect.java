...
public class LandMineSkillEffect implements ISkillEffect {
        ...
        this.definition = new HunterTrapManager.TrapDefinition(
                ...
                (trap, target) -> {
                    ...
                    if (trap.owner instanceof ServerPlayer player) {
                        damage += player.getCapability(PlayerStatsProvider.CAP)
                                .<Float>map(s -> s.getDEX() * 2.0f).orElse(0.0f);
                    }
    ...
    public void execute(ServerPlayer player, int level) {
        ...
        HunterTrapManager.placeTrap(player, (net.minecraft.server.level.ServerLevel) player.level(), pos.above(), definition);
